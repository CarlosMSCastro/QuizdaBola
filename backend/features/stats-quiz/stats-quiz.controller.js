...
exports.getStatsQuestion = async (req, res) => {
    try {
        ...
        if (format === 'F2') {
            return await handleF2(res, tableName, stat, statColumn, minimum, positions, difficulty, excludeList, bigTeamsOrder, BIG_TEAMS, minReqs);
        } else {
            return await handleF3(res, tableName, stat, statColumn, minimum, positions, difficulty, excludeList, BIG_TEAMS, minReqs, competitionId);
        }
...
async function handleF2(res, tableName, stat, statColumn, minimum, positions, difficulty, excludeList, bigTeamsOrder, BIG_TEAMS, minReqs) {
    // Buscar pool
    let pool = await service.getF2PlayerPool(tableName, stat, statColumn, minimum, positions, difficulty, excludeList, bigTeamsOrder, BIG_TEAMS, minReqs);
    ...
    if (!pair) {
        ...
        // FALLBACK: tentar com age
        if (stat !== 'age') {
            console.log('[FALLBACK FINAL] Tentando com stat=age...');
            const fallback = await service.fallbackToAge(tableName, excludeList, bigTeamsOrder, BIG_TEAMS, minReqs);
...
async function handleF3(res, tableName, stat, statColumn, minimum, positions, difficulty, excludeList, BIG_TEAMS, minReqs, competitionId) {
    ...
    const player = await service.getF3Player(tableName, stat, statColumn, minimum, positions, difficulty, excludeList, bigTeamFilter, bigTeamParams, minReqs, helpField);
