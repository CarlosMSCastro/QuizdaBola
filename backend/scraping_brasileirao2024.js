...
// Normalizar height e weight inline
...
async function scrape() {
    ...
    for (const team_id of teams) {
        ...
        do {
            ...
            for (const item of data.response) {
                ...
                // CRÍTICO: Filtrar apenas stats da Serie A (league_id: 71)
                const serieAStats = item.statistics.find(stat => stat.league.id === LEAGUE_ID);
                
                if (!serieAStats) {
                    skipped++;
                    continue; // Jogador não tem stats na Serie A, skip
                }
                ...
                await db.execute(
                    ...
                    [
                        ...
                        p.nationality ?? null,
                        normalizeHeight(p.height), // ← Normalizado
                        normalizeWeight(p.weight), // ← Normalizado
                        p.injured ? 1 : 0,
                        p.photo ?? null,
                        s.team?.id ?? null,
                        s.team?.name ?? null,
    ...
    console.log(`\n✅ Concluído!`);
    console.log(`📊 ${inserted} jogadores inseridos`);
